...
const thumb=v=>`<div class="thumb"><div class="ph" style="background:linear-gradient(135deg,hsl(${hue(v.id)},40%,25%),hsl(${(hue(v.id)+60)%360},40%,15%))">${esc(v.title[0])}</div><img src="https://i.ytimg.com/vi/${v.id}/hqdefault.jpg" loading="lazy" alt="" onerror="this.style.display='none'"><span class="dur ${v.dur==="LIVE"?"live":""}">${v.dur}</span></div>`;
...
const menuHtml=v=>`<button class="iconbtn more" data-menu="${v.id}" title="More"><svg viewBox="0 0 24 24"><path d="M12 16.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5-1.5-.67-1.5-1.5.67-1.5 1.5-1.5zM10.5 12c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5.67-1.5 1.5zm0-6c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5.67-1.5 1.5z"/></svg></button>
<div class="menu" id="menu-${v.id}">
  <div data-act="like" data-id="${v.id}"><svg viewBox="0 0 24 24"><path d="M18.77 11h-4.23l1.52-4.94C16.38 5.03 15.54 4 14.38 4c-.58 0-1.14.24-1.52.65L7 11H3v10h4h1h9.43c1.06 0 1.98-.67 2.19-1.61l1.34-6C21.23 12.15 20.18 11 18.77 11zM7 20H4v-8h3V20zM19.98 13.17l-1.34 6C18.54 19.65 18.03 20 17.43 20H8v-8.61l5.6-6.06C13.79 5.12 14.08 5 14.38 5c.26 0 .5.11.63.3.07.1.15.26.09.47l-1.52 4.94L13.18 12h1.35h4.23c.41 0 .8.17 1.03.46.12.15.25.4.19.71z"/></svg>${state.liked.includes(v.id)?"Remove from liked":"Add to liked videos"}</div>
  <div data-act="yt" data-id="${v.id}"><svg viewBox="0 0 24 24"><path d="M10 9.35 15 12l-5 2.65zM21.6 7.2c-.2-.9-.9-1.6-1.8-1.8C18.2 5 12 5 12 5s-6.2 0-7.8.4c-.9.2-1.6.9-1.8 1.8C2 8.8 2 12 2 12s0 3.2.4 4.8c.2.9.9 1.6 1.8 1.8C5.8 19 12 19 12 19s6.2 0 7.8-.4c.9-.2 1.6-.9 1.8-1.8.4-1.6.4-4.8.4-4.8s0-3.2-.4-4.8z"/></svg>Open on YouTube</div>
  <div data-act="ni" data-id="${v.id}"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 19c-4.96 0-9-4.04-9-9 0-2.08.72-3.99 1.9-5.52L17.52 19.1C15.99 20.28 14.08 21 12 21zm7.1-3.48L6.48 4.9C8.01 3.72 9.92 3 12 3c4.96 0 9 4.04 9 9 0 2.08-.72 3.99-1.9 5.52z"/></svg>Not interested</div>
</div>`;
...
function card(r){
  ...
  return `<div class="cardwrap"><div class="card" data-id="${v.id}">
    ...
    <div class="cbody">${avatar(v.ch)}<div class="cinfo">
      ...
      <div class="cmeta">${fmtViews(v.views)} views • ${esc(v.age)}</div>
      ${whyChip(r.why)}
    </div></div>
  </div>${menuHtml(v)}</div>`;
}
function listCard(r,extra=""){
  ...
  return `<div class="lcard card" data-id="${v.id}">
    ...
    <div class="cinfo">
      <div class="ctitle">${esc(v.title)}</div>
      <div class="cmeta">${fmtViews(v.views)} views • ${esc(v.age)}${extra}</div>
