...
// shell:true because on Windows these are often .cmd/.bat shims, which execFile can't
// launch directly — without it an installed yt-dlp still looks missing.
const WIN = process.platform === "win32";
const have = (cmd, args) => new Promise(res => execFile(cmd, args, { shell: WIN }, err => res(!err)));
// Which of the two tools this machine is missing, or null if both are there.
async function missingTools(){
  const [yt, ff] = await Promise.all([have("yt-dlp", ["--version"]), have("ffmpeg", ["-version"])]);
  return yt && ff ? null : [!yt && "yt-dlp", !ff && "ffmpeg"].filter(Boolean).join(" and ");
}
...
function download(id, fmt){
  return new Promise((resolve, reject) => {
    ...
    let out = "", err = "";
    const p = spawn("yt-dlp", args, { shell: WIN });
...
module.exports = async function handler(req, res){
  ...
  if(missing) return json(501, {error: "This server has no " + missing + " — downloads need them installed on the machine serving the page", missing});
